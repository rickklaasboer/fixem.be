...
describe('twitter adapter', () => {
    ...
    test('match covers twitter.com and x.com status URLs', () => {
        ...
        expect(
            a.match(new URL('https://twitter.com/janedoe/status/123/photo/1')),
        ).toBe(true);
        expect(
            a.match(new URL('https://mobile.twitter.com/janedoe/statuses/123')),
        ).toBe(true);
        expect(a.match(new URL('https://x.com/janedoe'))).toBe(false);
        expect(a.match(new URL('https://x.com/janedoe/status/123abc'))).toBe(
            false,
        );
        expect(
            a.match(new URL('https://xcom.example/janedoe/status/123')),
        ).toBe(false);
    });
    ...
    test('canonicalize strips trailing segments and query', () => {
        expect(
            a.canonicalize(
                new URL('https://twitter.com/janedoe/status/123/photo/1?s=20'),
            ),
        ).toBe('https://x.com/janedoe/status/123');
    });
    ...
    test('syndication token matches the reference algorithm', () => {
        ...
        expect(syndicationToken('1785342865283856000')).toBe(
            ((Number('1785342865283856000') / 1e15) * Math.PI)
                .toString(36)
                .replace(/(0+|\.)/g, ''),
        );
    });
    ...
    test('photo tweet: image kind, count marker, author', async () => {
        ...
        expect(requested[0]).toContain(
            `token=${syndicationToken('1785342865283856000')}`,
        );
