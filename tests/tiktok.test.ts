...
function pageHtml(fixture: unknown, scriptId = TIKTOK_DEFAULTS.rehydrationScriptId): string {
  return (
...
const VIDEO_URL = new URL("https://www.tiktok.com/@janetravels/video/7311234567890123456");
const PHOTO_URL = new URL("https://www.tiktok.com/@photoguy/photo/7322234567890123456");
...
describe("tiktok adapter", () => {
  ...
  test("match: exact hosts + full-post/short-link shapes only", () => {
    ...
    expect(a.match(new URL("https://m.tiktok.com/@u/video/123"))).toBe(true);
    ...
    expect(a.match(new URL("https://www.tiktok.com/t/ZMabcd123"))).toBe(true);
    // profile-only / root — no post segment
    expect(a.match(new URL("https://www.tiktok.com/@janetravels"))).toBe(false);
    ...
    expect(a.match(new URL("https://tiktok.example.com/@u/video/123"))).toBe(false);
    expect(a.match(new URL("https://x-tiktok.com/@u/video/123"))).toBe(false);
    expect(a.match(new URL("https://nottiktok.com/@u/video/123"))).toBe(false);
  });

  test("canonicalize: full posts normalize, short links kept raw", () => {
    expect(a.canonicalize(new URL("https://www.tiktok.com/@u/video/123?is_from_webapp=1"))).toBe(
      "https://www.tiktok.com/@u/video/123",
    );
    expect(a.canonicalize(new URL("https://m.tiktok.com/@u/photo/456?x=1"))).toBe(
      "https://www.tiktok.com/@u/photo/456",
    );
    // short links resolve at fetch time — canonicalize returns the raw URL
    expect(a.canonicalize(new URL("https://vm.tiktok.com/ZMabcd123/"))).toBe(
  ...
  test("short link: manual redirect -> aweme id -> page scrape", async () => {
    ...
    const fetchFn = (async (input: unknown, init?: RequestInit) => {
      ...
      recorded.push({ url, redirect: init?.redirect, headers: new Headers(init?.headers) });
    ...
    expect(recorded[1]!.url).toBe("https://www.tiktok.com/@janetravels/video/7311234567890123456");
    expect(m.kind).toBe("video");
    expect(m.originalUrl).toBe("https://www.tiktok.com/@janetravels/video/7311234567890123456");
  });
  ...
  test("short link: /v/<id>.html variant rewrites to /@i/video/<id>", async () => {
    ...
    const fetchFn = (async (input: unknown, init?: RequestInit) => {
      ...
      if (url.startsWith("https://vt.tiktok.com/")) {
        return new Response(null, {
          status: 301,
          headers: { Location: "https://m.tiktok.com/v/7311234567890123456.html" },
        });
      }
      return new Response(pageHtml(videoFixture), { status: 200 });
    }) as unknown as FetchFn;
    ...
    expect(recorded[1]!.url).toBe("https://www.tiktok.com/@i/video/7311234567890123456");
    expect(m.kind).toBe("video");
  });

  test("short link that does not redirect throws", async () => {
    const fetchFn = (async () => new Response(null, { status: 200 })) as unknown as FetchFn;
    const ad = createTiktokAdapter(fetchFn);
    expect(ad.resolve(new URL("https://vm.tiktok.com/ZMabcd123"))).rejects.toThrow();
  });
  ...
  test("video: highest-bitrate PlayAddr, proxyHeaders, dims, poster, ttl", async () => {
    ...
    expect(m.video?.url).toBe("https://v16-webapp.tiktokcdn.com/high_1200.mp4?a=1");
    // RAW play URL is emitted — the app wraps it in the /v/ proxy (T25)
    expect(m.video?.url.startsWith("https://v16-webapp.tiktokcdn.com/")).toBe(true);
    ...
    expect(m.image?.url).toBe("https://p16-sign.tiktokcdn.com/cover_720.jpg?x-expires=1");
  ...
  test("video: falls back to video.playAddr when bitrateInfo is empty", async () => {
    ...
    noBitrate.__DEFAULT_SCOPE__["webapp.video-detail"].itemInfo.itemStruct.video.bitrateInfo = [];
    const ad = createTiktokAdapter(pageFetch(noBitrate));
    const m = await ad.resolve(VIDEO_URL);
    expect(m.kind).toBe("video");
    expect(m.video?.url).toBe("https://v16-webapp.tiktokcdn.com/fallback_play.mp4?a=1");
  });
  ...
  test("photo post: first image + count marker", async () => {
    ...
    expect(m.image?.url).toBe("https://p16-sign.tiktokcdn.com/photo1_full.jpg?x-expires=1");
    expect(m.title).toBe("Photo Guy (@photoguy)");
    expect(m.description).toBe("Weekend photo dump 📷 3 images");
    expect(m.video).toBeUndefined();
    expect(m.originalUrl).toBe("https://www.tiktok.com/@photoguy/photo/7322234567890123456");
  });

  test("restricted statusCode -> informative link embed (no throw)", async () => {
    for (const code of [209002, 209004]) {
      const restricted = { __DEFAULT_SCOPE__: { "webapp.video-detail": { statusCode: code } } };
  ...
  test("not-found statusCode 10204 throws", async () => {
    const notFound = { __DEFAULT_SCOPE__: { "webapp.video-detail": { statusCode: 10204 } } };
  ...
  test("missing rehydration script throws", async () => {
    const fetchFn = (async () =>
      new Response("<html><body>no data here</body></html>", { status: 200 })) as unknown as FetchFn;
    const ad = createTiktokAdapter(fetchFn);
    expect(ad.resolve(VIDEO_URL)).rejects.toThrow();
  });

  test("non-OK page response throws", async () => {
    const fetchFn = (async () => new Response("blocked", { status: 403 })) as unknown as FetchFn;
    const ad = createTiktokAdapter(fetchFn);
    expect(ad.resolve(VIDEO_URL)).rejects.toThrow();
  });

  test("TIKTOK_DEFAULTS wires the web + (future) mobile-API config fields", () => {
    expect(TIKTOK_DEFAULTS.rehydrationScriptId).toBe("__UNIVERSAL_DATA_FOR_REHYDRATION__");
  ...
  test("config is injectable: custom rehydrationScriptId is used for extraction", async () => {
    ...
    const ad = createTiktokAdapter(fetchFn, { ...TIKTOK_DEFAULTS, rehydrationScriptId: "__CUSTOM_DATA__" });
    const m = await ad.resolve(VIDEO_URL);
    expect(m.kind).toBe("video");
    expect(m.title).toBe("Jane Traveler (@janetravels)");
  });
