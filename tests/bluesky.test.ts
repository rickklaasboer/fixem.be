...
function fakeFetch(thread: unknown, requested: string[] = []): FetchFn {
    return (async (input: Parameters<FetchFn>[0]) => {
        ...
        if (url.includes('resolveHandle'))
            return new Response(JSON.stringify(resolveHandle));
...
describe('bluesky adapter', () => {
    ...
    test('match requires profile/post path shape', () => {
        ...
        expect(
            a.match(new URL('https://bsky.app/profile/alice.bsky.social')),
        ).toBe(false);
        expect(a.match(new URL('https://bsky.example/profile/a/post/b'))).toBe(
            false,
        );
    });
    ...
    test('canonicalize strips query', () => {
        expect(
            a.canonicalize(
                new URL(
                    'https://bsky.app/profile/alice.bsky.social/post/3kabc?ref=share',
                ),
            ),
        ).toBe('https://bsky.app/profile/alice.bsky.social/post/3kabc');
    });
    ...
    test('resolves handle to did, then fetches thread', async () => {
        ...
        expect(requested[1]).toContain(
            encodeURIComponent(
                'at://did:plc:abc123xyz/app.bsky.feed.post/3kabc',
            ),
        );
        expect(m.kind).toBe('image');
        expect(m.title).toBe('Alice (@alice.bsky.social)');
        expect(m.image?.url).toBe(
            'https://cdn.bsky.app/img/feed_fullsize/plain/cat1@jpeg',
        );
    ...
    test('video post uses thumbnail image, no video field (HLS)', async () => {
        ...
        expect(m.image?.url).toBe(
            'https://video.bsky.app/watch/did/xyz/thumbnail.jpg',
        );
    ...
    test('quote post appends quoted text', async () => {
        ...
        expect(m.description).toBe(
            'This is important: ↪ @dave.bsky.social: Original insight about embeds.',
        );
    });
    ...
    test('external embed uses thumb and link title', async () => {
        ...
        expect(m.image?.url).toBe(
            'https://cdn.bsky.app/img/feed_thumbnail/plain/ext@jpeg',
        );
    ...
    test('content labels map to nsfw', async () => {
        const labeled = structuredClone(postImages) as {
    ...
    test('empty displayName titles as just @handle', async () => {
        const noName = structuredClone(postImages) as {
