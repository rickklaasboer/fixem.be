...
import {
    minimalMeta,
    renderMetaHtml,
    renderPreviewNoAdapter,
    renderPreviewReport,
} from './render/meta-html';
...
export function buildApp(deps: AppDeps): Hono {
    ...
    async function withProxiedVideo(
        meta: EmbedMetadata,
        ...
        if (
            u.protocol !== 'https:' ||
            !isHostAllowed(u.hostname, config.proxyHostAllowlist)
        ) {
            logger.warn(
                {host: u.hostname},
        ...
        return {
            ...meta,
            video: {...vid, url: `${config.publicBaseUrl}/v/${token}`},
        };
    }
    ...
    app.get('/healthz', async (c) =>
        c.json({ok: true, redis: await cache.ping()}),
    );
