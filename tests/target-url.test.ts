...
import TargetUrl, {type ParsedTarget} from '../src/support/TargetUrl';

function ok(pathname: string, search = ''): string {
    const r = TargetUrl.parse(pathname, search);
    if (!r.ok) throw new Error(`expected ok, got ${r.reason}`);
    return r.url.href;
}

describe('TargetUrl.parse', () => {
    ...
    test('double-encoded input is rejected', () => {
        const r = TargetUrl.parse('/https%253A%252F%252Fexample.com%252Fx', '');
        expect(r.ok).toBe(false);
        if (!r.ok) expect(r.reason).toBe('double-encoded');
    });

    test('garbage is unparseable', () => {
        expect(TargetUrl.parse('/not a url at all', '').ok).toBe(false);
        expect(TargetUrl.parse('/favicon.ico', '').ok).toBe(false);
    });

    test('non-http scheme is rejected', () => {
        const r = TargetUrl.parse('/ftp://example.com/file', '');
        expect(r.ok).toBe(false);
    ...
    test('bare host without a path segment is unparseable', () => {
        const r = TargetUrl.parse('/example.com', '');
        expect(r.ok).toBe(false);
