...
@injectable()
export default class BlueskyAdapter extends BaseAdapter {
    public name = 'bluesky';
    ...
    /**
     * Resolve a Bluesky post into embed metadata.
     */
    public async resolve(
        ...
        if (!did.startsWith('did:')) {
            const r = await this.http.getJson<{did: string}>(
        ...
        const data = await this.http.getJson<BskyThreadResponse>(
        ...
        const descParts: string[] = text ? [Text.truncate(text, 300)] : [];
        ...
        if (embedType.startsWith('app.bsky.embed.images')) {
            ...
            if (quoted?.author?.handle) {
                ...
                descParts.push(
                    qText
                        ? `↪ @${quoted.author.handle}: ${Text.truncate(qText, 120)}`
