...
function proxyable(url: URL, allowlist: string[]): boolean {
    return (
        url.protocol === 'https:' &&
        VideoProxy.isHostAllowed(url.hostname, allowlist)
    );
}
        ...
        async pull(controller) {
            ...
            try {
                ...
                const idle = new Promise<never>((_, reject) => {
                    timer = setTimeout(
                        () => reject(new Error('proxy idle timeout')),
                        idleMs,
                    );
                });
...
/**
 * Streams the `/v/:token` media proxy: rate-limits every client, verifies the
 * signed token, re-validates each redirect hop against the host allowlist +
 * https-only, and bounds concurrency/bytes/idle-time on the transfer. Exists
 * for CDNs whose media requires request headers Discord's crawler won't send.
 */
@singleton()
export default class ProxyStreamer {
    private inflight = 0;
    ...
    public async stream(c: Context): Promise<Response> {
        ...
        const hits = await this.rateLimitStore.hit(
        ...
        const payload = await this.signer.verify(
            ...
            // fallback only matters if that ever isn't true, and verify() rejects
            // it as any other malformed token → 404, same as "not found" today.
            c.req.param('token') ?? '',
            this.clock.now(),
        );
        ...
        try {
            ...
            let up: Response;
            for (let hop = 0; ; hop++) {
                up = await this.http.fetch(current.href, {
                    headers: fwd,
                    redirect: 'manual',
                    signal: abort.signal,
                });
                ...
                if (!loc || hop >= MAX_REDIRECTS) {
                    this.logger.warn(
                        {host: current.hostname, status: up.status},
                        'proxy redirect not followable',
                    );
            ...
            this.logger.warn(
                {host: target.hostname, err: String(err)},
                'proxy fetch failed',
            );
            return c.text('bad gateway', 502);
        } finally {
