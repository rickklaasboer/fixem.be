...
/**
 * Renders the crawler-facing meta-refresh HTML (OG/Twitter-card tags) and
 * the human-facing `/preview/` diagnostic pages, from a single
 * `EmbedMetadata`.
 */
@singleton()
export default class MetaHtmlRenderer {
    ...
    private static tag(
        ...
        return `<meta ${kind}="${key}" content="${MetaHtmlRenderer.esc(String(value))}">`;
    }
    ...
    private static readonly FONT_LINKS =
...
<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@700&family=DM+Sans:wght@400;700&display=swap" rel="stylesheet">`;
    private static readonly SANS = `"DM Sans",system-ui,sans-serif`;
    private static readonly TITLE_FONT = `"Montserrat",sans-serif`;
    private static readonly MONO = `ui-monospace,"JetBrains Mono",monospace`;

    /**
     * Preview-only diagnostic page for a URL that matched no adapter.
     */
    public previewNoAdapter(targetUrl: string): string {
        const esc = MetaHtmlRenderer.esc;
        const FONT_LINKS = MetaHtmlRenderer.FONT_LINKS;
        const SANS = MetaHtmlRenderer.SANS;
        const TITLE_FONT = MetaHtmlRenderer.TITLE_FONT;
        const MONO = MetaHtmlRenderer.MONO;
        const SUPPORTED = MetaHtmlRenderer.SUPPORTED;
        return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<p>Profile pages, home feeds, and other non-post URLs won't match — only a single post or clip does.</p>
    ...
    private static dims(m: {width?: number; height?: number}): string {
        return m.width && m.height ? ` (${m.width}×${m.height})` : '';
    }

    /**
     * Rich debug view for `/preview/` on a matched URL: the resolution
     * outcome, a visual embed card, the parsed metadata, and the exact
     * crawler HTML Discord would receive. Not crawler-facing — humans only
     * reach it via `/preview/`.
     */
    public previewReport(opts: {
        platform: string;
        status: 'ok' | 'degraded';
        cacheHit?: boolean;
        reason?: string;
        ...
        const esc = MetaHtmlRenderer.esc;
        const dims = MetaHtmlRenderer.dims;
        const FONT_LINKS = MetaHtmlRenderer.FONT_LINKS;
        const SANS = MetaHtmlRenderer.SANS;
        const TITLE_FONT = MetaHtmlRenderer.TITLE_FONT;
        const MONO = MetaHtmlRenderer.MONO;
        ...
        const crawlerHtml = this.render(meta, {
...
<pre>${esc(crawlerHtml)}</pre>
    ...
    /**
     * Minimal `EmbedMetadata` fallback for a URL that resolved to nothing
     * more specific than its own canonical link.
     */
    public minimalMeta(canonicalUrl: string): EmbedMetadata {
        return {
            kind: 'link',
            title: canonicalUrl,
            siteName: 'fixem.be',
    ...
    /**
     * Render the crawler-facing meta-refresh HTML (OG/Twitter-card tags,
     * oEmbed discovery link, and a meta-refresh redirect) for one
     * `EmbedMetadata`.
     */
    public render(
        meta: EmbedMetadata,
        ...
        const esc = MetaHtmlRenderer.esc;
        const tag = MetaHtmlRenderer.tag;
        ...
        if (meta.image) {
            ...
            if (meta.image.height)
                lines.push(
                    tag('property', 'og:image:height', meta.image.height),
                );
        }
