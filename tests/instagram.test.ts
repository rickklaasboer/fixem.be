...
function fakeFetch(
    ...
    return (async (input: unknown, init?: RequestInit) => {
        ...
        if (opts.raw !== undefined)
            return new Response(opts.raw, {status: opts.status ?? 200});
        return new Response(JSON.stringify(opts.body ?? imageFixture), {
...
describe('instagram adapter', () => {
    ...
    test('match: exact hosts + post/reel/reels/tv path shapes only', () => {
        ...
        expect(a.match(new URL('https://instagram.com/p/CabcDEF123'))).toBe(
            true,
        );
        expect(
            a.match(new URL('https://www.instagram.com/reel/CabcDEF123')),
        ).toBe(true);
        expect(
            a.match(new URL('https://www.instagram.com/reels/CabcDEF123')),
        ).toBe(true);
        expect(
            a.match(new URL('https://www.instagram.com/tv/CabcDEF123')),
        ).toBe(true);
        expect(a.match(new URL('https://ddinstagram.com/p/CabcDEF123'))).toBe(
            true,
        );
        // profile-only / stories / no code
        expect(a.match(new URL('https://www.instagram.com/janedoe'))).toBe(
            false,
        );
        expect(a.match(new URL('https://www.instagram.com/p/'))).toBe(false);
        expect(
            a.match(new URL('https://www.instagram.com/stories/janedoe/123')),
        ).toBe(false);
        // wrong hosts (substring / subdomain must not match)
        expect(
            a.match(new URL('https://instagram.com.evil.com/p/CabcDEF123')),
        ).toBe(false);
        expect(a.match(new URL('https://notinstagram.com/p/CabcDEF123'))).toBe(
            false,
        );
        expect(a.match(new URL('https://foo.instagram.com/p/CabcDEF123'))).toBe(
            false,
        );
    });
    ...
    test('canonicalize: reel/reels/tv normalized to /p/, query stripped', () => {
        expect(
            a.canonicalize(
                new URL('https://www.instagram.com/reel/CabcDEF123?igshid=x'),
            ),
        ).toBe('https://www.instagram.com/p/CabcDEF123');
        expect(
            a.canonicalize(
                new URL('https://www.instagram.com/reels/CabcDEF123'),
            ),
        ).toBe('https://www.instagram.com/p/CabcDEF123');
        expect(
            a.canonicalize(new URL('https://www.instagram.com/tv/CabcDEF123')),
        ).toBe('https://www.instagram.com/p/CabcDEF123');
        expect(
            a.canonicalize(new URL('https://ddinstagram.com/p/CabcDEF123?x=1')),
        ).toBe('https://www.instagram.com/p/CabcDEF123');
    });
    ...
    test('graphql request: endpoint, headers, and form body shape', async () => {
        ...
        expect(r.headers.get('X-FB-Friendly-Name')).toBe(
        ...
        expect(r.body).toContain(
            encodeURIComponent('{"shortcode":"CabcDEF123"}'),
        );
    });
    ...
    test('image post: image kind, title, caption, dims, author, meta', async () => {
        ...
        expect(m.image?.url).toBe(
            'https://scontent.cdninstagram.com/ig_image_full.jpg?efg=sig',
        );
    ...
    test('video post: raw cdn url + proxyHeaders, dims, poster, ttl', async () => {
        ...
        const m = await ad.resolve(
            new URL('https://www.instagram.com/reel/CabcDEF456'),
        );
        expect(m.kind).toBe('video');
        // RAW cdn URL is emitted; proxy-wrapping is the app's job (T25).
        expect(m.video?.url).toBe(
            'https://scontent.cdninstagram.com/ig_video_720.mp4?efg=sig&_nc_ht=x',
        );
        expect(
        ...
        expect(m.video?.proxyHeaders?.Referer).toBe(
            'https://www.instagram.com/',
        );
        expect(m.image?.url).toBe(
            'https://scontent.cdninstagram.com/ig_video_poster.jpg?efg=sig',
        );
        expect(m.ttlSeconds).toBe(3600);
    ...
    test('modern XDT-prefixed typenames resolve like the legacy names', async () => {
                        ...
                        edges: [
                            {
                                node: {
                                    ...
                                    display_url:
                                        'https://scontent.cdninstagram.com/s1.jpg',
                                },
                            },
                            {
                                node: {
                                    __typename: 'XDTGraphImage',
                                    display_url:
                                        'https://scontent.cdninstagram.com/s2.jpg',
                                },
                            },
                        ],
                    },
        ...
        const m2 = await ad2.resolve(
            new URL('https://www.instagram.com/p/CabcDEF456'),
        );
    ...
    test('login wall (require_login) -> link embed', async () => {
        const ad = createInstagramAdapter(
            fakeFetch({body: {require_login: true}}),
        );
        const m = await ad.resolve(P_URL);
        expect(m.kind).toBe('link');
    ...
    test('non-JSON body (HTML login page) -> link embed, no throw', async () => {
        const ad = createInstagramAdapter(
            fakeFetch({raw: '<!DOCTYPE html><html>login</html>'}),
        );
        const m = await ad.resolve(P_URL);
        expect(m.kind).toBe('link');
        expect(m.description).toContain('login wall');
    ...
    test('older alias data.shortcode_media is honored', async () => {
        ...
        expect(m.image?.url).toBe(
            'https://scontent.cdninstagram.com/legacy.jpg',
        );
    });
                        ...
                        edges: [
                            ...
                            },
                            {
                                node: {
                                    __typename: 'GraphImage',
                                    display_url:
                                        'https://scontent.cdninstagram.com/c3.jpg',
                                },
                            },
                        ],
                    },
                        ...
                        edges: [
                            {
                                node: {
                                    ...
                                    video_url:
                                        'https://scontent.cdninstagram.com/child_video.mp4',
                                    display_url:
                                        'https://scontent.cdninstagram.com/child_poster.jpg',
    ...
    function mobileFetch(item: unknown, recorded?: Recorded[]): FetchFn {
        return (async (input: unknown, init?: RequestInit) => {
            ...
            throw new Error(
                'graphql should not be called when the mobile API resolves',
            );
        }) as unknown as FetchFn;
    }
    ...
    test('with a cookie, the mobile API is used and carries the cookie + X-CSRFToken', async () => {
        ...
        const item = {
            ...
            image_versions2: {
                candidates: [
                    {
                        url: 'https://scontent.cdninstagram.com/mob.jpg',
                        width: 1080,
                        height: 1080,
                    },
                ],
        ...
        expect(recorded[0]!.headers.get('Cookie')).toBe(
            'sessionid=SECRET_SESSION; csrftoken=TOKEN123; ds_user_id=42',
        );
    ...
    test('mobile API: video item → proxied video, poster, dims', async () => {
        ...
        const ad = createInstagramAdapter(mobileFetch(item), {
            ...INSTAGRAM_DEFAULTS,
            cookie: 'sessionid=X',
        });
        const m = await ad.resolve(P_URL);
    ...
    test('mobile API: carousel (media_type 8) → first child + count', async () => {
        ...
        const m = await createInstagramAdapter(mobileFetch(item), {
            ...INSTAGRAM_DEFAULTS,
            cookie: 'sessionid=X',
        }).resolve(P_URL);
        expect(m.image?.url).toBe('https://scontent.cdninstagram.com/c1.jpg');
    ...
    test('no mobile call / no Cookie header when no session cookie is configured', async () => {
        const recorded: Recorded[] = [];
        await createInstagramAdapter(
            fakeFetch({recorded, body: videoFixture}),
        ).resolve(P_URL);
    ...
    test('cookie set but mobile API fails → falls through to snapsave/degrade (no crash)', async () => {
        ...
        const m = await createInstagramAdapter(fetchFn, {
            ...INSTAGRAM_DEFAULTS,
            cookie: 'sessionid=X',
        }).resolve(P_URL);
        expect(m.kind).toBe('link');
    ...
    test('snapsave fallback fires when our own fetch is login-walled (opt-in)', async () => {
        const blob = await Bun.file(
            new URL(
                './fixtures/instagram/snapsave-response.txt',
                import.meta.url,
            ),
        ).text();
        // graphql → login wall; snapsave.app → the recorded blob
        const fetchFn = (async (input: unknown) => {
            if (String(input).includes('snapsave'))
                return new Response(blob, {status: 200});
            return new Response(
                JSON.stringify({status: 'fail', require_login: true}),
                {status: 200},
            );
        }) as unknown as FetchFn;
        const ad = createInstagramAdapter(fetchFn, {
            ...INSTAGRAM_DEFAULTS,
            snapsave: true,
        });
        const m = await ad.resolve(P_URL);
    ...
    test('config is injectable (2nd param overrides defaults)', async () => {
        ...
        expect(recorded[0]!.headers.get('X-FB-Friendly-Name')).toBe(
            'CustomQuery',
        );
    });
});
