...
function fakeFetch(
  ...
  return (async (input: unknown, init?: RequestInit) => {
    ...
    opts.recorded?.push({ url, body: init?.body?.toString(), headers: new Headers(init?.headers) });
...
describe("threads adapter", () => {
  ...
  test("match: exact hosts + post/short path shapes only", () => {
    ...
    expect(a.match(new URL("https://threads.net/@johndoe/post/ABC123"))).toBe(true);
    expect(a.match(new URL("https://www.threads.net/@johndoe/post/ABC123"))).toBe(true);
    ...
    expect(a.match(new URL("https://www.threads.com/@johndoe"))).toBe(false);
    // wrong hosts (substring / subdomain must not match)
    expect(a.match(new URL("https://threads.example.com/@johndoe/post/ABC123"))).toBe(false);
    expect(a.match(new URL("https://x-threads.com/@johndoe/post/ABC123"))).toBe(false);
  ...
  test("canonicalize: strips query, forces threads.com, keeps /t/ form", () => {
    expect(a.canonicalize(new URL("https://threads.net/@johndoe/post/ABC123?igshid=xyz"))).toBe(
      "https://www.threads.com/@johndoe/post/ABC123",
    );
    expect(a.canonicalize(new URL("https://www.threads.com/t/ABC123?x=1"))).toBe(
  ...
  test("two-call ordering: route -> graphql carrying the resolved post_id", async () => {
    ...
    expect(recorded[1]!.headers.get("X-FB-Friendly-Name")).toBe(THREADS_DEFAULTS.friendlyName);
    expect(recorded[1]!.headers.get("X-IG-App-ID")).toBe(THREADS_DEFAULTS.appId);
  });
  ...
  test("image post: image kind, title, author, description", async () => {
    ...
    expect(m.image?.url).toBe("https://scontent.cdninstagram.com/image_full.jpg?efg=sig");
  ...
  test("video post: raw cdn url + proxyHeaders, dims, poster, ttl", async () => {
    ...
    expect(m.video?.url).toBe("https://scontent.cdninstagram.com/video_720.mp4?efg=sig&_nc_ht=x");
    ...
    expect(m.image?.url).toBe("https://scontent.cdninstagram.com/video_poster.jpg?efg=sig");
            ...
            thread_items: [
              {
                post: {
                  ...
                  carousel_media: [
                    {
                      image_versions2: { candidates: [{ url: "https://scontent.cdninstagram.com/c1.jpg", width: 800, height: 800 }] },
                      original_width: 800,
                      original_height: 800,
                    },
                    { image_versions2: { candidates: [{ url: "https://scontent.cdninstagram.com/c2.jpg" }] } },
                    { image_versions2: { candidates: [{ url: "https://scontent.cdninstagram.com/c3.jpg" }] } },
                  ],
                },
              },
            ],
    ...
    const withDangling = {
      data: {
        data: {
          containing_thread: {
            thread_items: [
              { post: postImage.data.data.containing_thread.thread_items[0]!.post },
  ...
  test("config is injectable (2nd param overrides defaults)", async () => {
    ...
    expect(recorded[1]!.headers.get("X-FB-Friendly-Name")).toBe("CustomQuery");
  });
});
