...
- `src/app.ts` — the catch-all route parses the wrapped target URL, branches on `User-Agent`: known crawlers (or `?fixem=preview`) get resolved + rendered embed HTML; everyone else gets a `302` (no resolve on that path). Rate-limits non-crawlers. `onError` + the resolver guarantee a well-formed URL **never** 500s. Also mounts the `/v/` proxy and, at render time, rewrites `video.proxyHeaders` media into signed `/v/` URLs (`withProxiedVideo`).
