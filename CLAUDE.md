...
- `src/http/controllers/ApiV1Controller.ts` — the public `/api/v1/*` JSON API (`resolve` GET + POST batch, `canonical`, `platforms`, `health`). Thin: reuses `Resolver.resolve()`/`canonicalFor()` verbatim and maps outcomes through `PublicMetaRenderer` (the sole place `video.proxyHeaders` is stripped and `needsProxy`/`playableUrl` derived). Replaces the old `/api/status/adapter` (its payload lives in `health`). The contract is `openapi.yaml` (repo root), served publicly at `GET /openapi.yaml` and guarded against route drift by `tests/openapi.test.ts`.
- `src/http/middleware/*` — `ApiAuthMiddleware` gates `/api/v1/*` with `Authorization: Bearer` against the `API_KEYS` set (closed 404 when none configured); `ApiRateLimitMiddleware` rate-limits `/api/v1/*` **per key** (hash-bucketed, no crawler bypass) and emits `X-RateLimit-*`; `RateLimitMiddleware` limits `/oembed` + `*` **per IP** (known crawlers bypass; `/preview/` does **not**). `/v/` limits **all** clients unconditionally inside `ProxyStreamer`.
...
- `src/render/{MetaHtmlRenderer,OembedRenderer,PublicMetaRenderer}.ts` — build the crawler HTML + oEmbed JSON + public API JSON from one `EmbedMetadata`. `MetaHtmlRenderer` escapes every interpolated value; `PublicMetaRenderer` strips `video.proxyHeaders` and only attaches a signed `playableUrl` on `?media=proxied`.
