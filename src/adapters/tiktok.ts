...
export function createTiktokAdapter(
    ...
    function extractItemDetail(html: string): {
        statusCode?: number;
    ...
    return {
        ...
        match(url) {
            ...
            if (MAIN_HOSTS.has(url.hostname))
                return (
                    POST_RE.test(url.pathname) ||
                    SHORT_PATH_RE.test(url.pathname)
                );
            return false;
        },
        canonicalize(url) {
        ...
        async resolve(url, signal): Promise<EmbedMetadata> {
            ...
            if (isShortLink(url)) {
                ...
                if (!full)
                    throw new Error(
                        'tiktok: short link did not resolve to a post',
                    );
                pageUrl = full;
            } else {
            ...
            const {statusCode, itemStruct} = extractItemDetail(
                await res.text(),
            );
            ...
            if (photos && photos.length > 0) {
                ...
                if (photos.length > 1)
                    descParts.push(`📷 ${photos.length} images`);
